...
export default function Home() {
  ...
  const navItems = [
    { id: "overview", label: "대시보드", href: "/dashboard", icon: Gauge },
    { id: "trends", label: "관광 트렌드", href: "/trends", icon: TrendingUp },
    { id: "products", label: "관광상품 기획", href: "/products", icon: Sparkles },
  ];
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <Link href="/" className="brand"><div className="brand-mark"><span>●</span><span>●</span><span>●</span></div><div><strong>trip ping</strong><small>INSIGHT PORTAL</small></div></Link>
        ...
        <nav className="side-nav">{navItems.map(({ id, label, href, icon: Icon }) => <button key={id} className={activeNav === id ? "active" : ""} onClick={() => { setActiveNav(id); window.location.href = href; }}><Icon size={18} /><span>{label}</span>{id === "trends" && <em>3</em>}</button>)}</nav>
        <div className="side-label spaced">WORKSPACE</div>
        <nav className="side-nav"><button onClick={() => { window.location.href = "/reports"; }}><FileText size={18} /><span>보고서 만들기</span></button><button onClick={() => { window.location.href = "/settings"; }}><Settings2 size={18} /><span>조직 설정</span></button></nav>
        <div className="sidebar-bottom"><div className="help-card"><CircleHelp size={17} /><div><b>도움이 필요하신가요?</b><span>가이드 센터에서 확인하세요</span></div><ChevronRight size={15} /></div><button className="account-row" onClick={() => logout()}><div className="profile-avatar">김</div><div><b>김지훈 님</b><span>로그아웃</span></div><Settings2 size={16} /></button></div>
      </aside>
